var cb = {
  ...
  toast: (status, head, body) => {
    ...
    if (status == 1 || status == "1" || status == true) {
      ticon.innerHTML = "thumb_up";
    } else if (status == 0 || status == "0" || status == false) {
      ticon.innerHTML = "error";
    } else {
      ticon.innerHTML = "help";
    }
  ...
  ajax: (opt) => {
    ...
    if (opt.url === undefined) {
      err = "Target URL is not set!";
    }
    if (opt.onpass === undefined) {
      err = "Function to call on onpass is not set!";
    }
    if (err !== null) {
  ...
  api: (opt) => {
    ...
    if (opt.passmsg === undefined) {
      opt.passmsg = "OK";
    }
    if (opt.nofail === undefined) {
      opt.nofail = false;
    }
    ...
    options.onpass = (res) => {
      ...
      if (res.status == "E") {
        ...
        if (!opt.nofail) {
          cb.modal("ERROR", res.message);
        }
        if (opt.onfail) {
          opt.onfail(res.message);
        }
      }
    };
